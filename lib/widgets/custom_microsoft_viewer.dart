...
class CustomMicrosoftViewerState extends State<CustomMicrosoftViewer> {
  ...
  Future<void> parseAndShowData() async {
    try {
      ...
      if (fileType == "word") {
        var relFile = archive.singleWhere(
          (f) => f.name.endsWith("document.xml.rels"),
        );
        getRelationships(relFile);
        archive
            .where((f) => f.name.startsWith('word/media/'))
            .forEach((f) => extractMedia(f, wordOutputDirectory));
        var stylesFile = archive.singleWhere(
          (f) => f.name.endsWith("word/styles.xml"),
        );
        ...
        if (defaultValues.isNotEmpty) {
          if (defaultValues["fontSize"] != null)
            wordDocument.defaultFontSize = int.parse(
              defaultValues["fontSize"]!,
            );
          if (defaultValues["lineSpacing"] != null)
            wordDocument.defaultLineSpacing = int.parse(
              defaultValues["lineSpacing"]!,
            );
        }
        var fontTable = archive.singleWhereOrNull(
          (f) => f.name.endsWith("word/fontTable.xml"),
        );
        if (fontTable != null) {
          var fontTableRel = archive.singleWhereOrNull(
            (f) => f.name.endsWith("_rels/fontTable.xml.rels"),
          );
          if (fontTableRel != null) {
            CommonProcessor().processFonts(fontList, fontTable, fontTableRel);
            for (var font in fontList) {
              var fontFile = archive.singleWhereOrNull(
                (f) => f.name.endsWith(font.fileName),
              );
              if (fontFile != null)
                await loadFonts(
                  fontFile,
                  font.name,
                  font.fontKey.replaceAll("{", "").replaceAll("}", ""),
                );
            }
          }
        }
        var footNoteFile = archive.singleWhereOrNull(
          (f) => f.name.endsWith("footnotes.xml"),
        );
        var endNoteFile = archive.singleWhereOrNull(
          (f) => f.name.endsWith("endnotes.xml"),
        );
        WordProcessor().processFootEndNotes(
          footNoteFile,
          endNoteFile,
          footNotes,
          endNotes,
        );
        var wordFile = archive.singleWhere(
          (f) => f.name == 'word/document.xml',
        );
        WordProcessor().processWordFile(
          wordFile,
          elementDepth,
          relationShips,
          wordOutputDirectory,
        ...
        List<Widget> tempWidgets = WordProcessor().displayWordFile(
          fileType,
          wordDocument,
          stylesList,
          footNotes,
        ...
        var relFile = archive.singleWhere(
          (f) => f.name.endsWith("workbook.xml.rels"),
        );
        getRelationships(relFile);
        var shareStringsFile = archive.singleWhere(
          (f) => f.name.endsWith("sharedStrings.xml"),
        );
        getSharedStrings(shareStringsFile);
        var workbookFile = archive.singleWhere(
          (f) => f.name.endsWith("xl/workbook.xml"),
        );
        SpreadsheetProcessor().getSpreadSheetDetails(workbookFile, spreadSheet);
        SpreadsheetProcessor().readAllSheets(
          spreadSheet,
          relationShips,
          archive,
        );
        List<Widget> tempWidgets = await SpreadsheetProcessor()
            .displaySpreadSheet(spreadSheet, sharedStrings);
        if (mounted)
          setState(() {
        ...
        var relFile = archive.singleWhere(
          (f) => f.name.endsWith("presentation.xml.rels"),
        );
        getRelationships(relFile);
        archive
            .where((f) => f.name.startsWith('ppt/media/'))
            .forEach((f) => extractMedia(f, presentationOutputDirectory));
        var presentationFile = archive.singleWhere(
          (f) => f.name.endsWith("ppt/presentation.xml"),
        );

        CustomPresentationProcessor().getPresentationDetails(
          presentationFile,
          presentation,
        );
        CustomPresentationProcessor().readAllSlides(
          presentation,
          relationShips,
          archive,
        ...
        List<Widget> tempWidgets = await CustomPresentationProcessor()
            .displayPresentation(presentation, sw);
        if (mounted)
          setState(() {
  ...
  void getSharedStrings(ArchiveFile shareStringsFile) {
    ...
    sharedStrings = document
        .findAllElements('si')
        .indexed
        .map((e) => SharedString(e.$1, e.$2.getElement("t")?.innerText ?? ""))
        .toList();
  }
  ...
  void extractMedia(ArchiveFile mediaFile, String dirPath) {
    ...
    ).writeAsBytesSync(mediaFile.content as List<int>);
  }
  ...
  Future<void> loadFonts(
    ...
    await (FontLoader(
      fontFamily,
    )..addFont(Future.value(ByteData.view(fontFile.content.buffer)))).load();
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (fileType == "word")
      ...
      content = spreadSheetWidgets;
    else if (fileType == "presentation")
      content = presentationWidgets;
