...
class InstructorAssessmentsScreen extends StatefulWidget {
  ...
  @override
  State<InstructorAssessmentsScreen> createState() =>
      _InstructorAssessmentsScreenState();
}

class _InstructorAssessmentsScreenState
    extends State<InstructorAssessmentsScreen> {
  ...
  void initState() {
    ...
    _selectedCourseIdFilter =
        widget.initialCourseId; // Initialize with initialCourseId if provided
    _fetchData();
  }
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      final courses = await _apiService.getInstructorCourses().timeout(
      ...
      for (var course in courses) {
        ...
        final stats = await _apiService
            .getCourseEnrollmentStats(course['id'].toString())
      ...
      for (var course in courses) {
        final assessments = await _apiService
            .getAssessments(course['id'].toString())
            .timeout(const Duration(seconds: 15));
  ...
  void _showCreateAssessmentSheet() {
    ...
    String? selectedCourseId =
        _selectedCourseIdFilter ??
        (_classes.isNotEmpty ? _classes[0]["id"].toString() : null);
    ...
    Future<void> updateGroupsForCourse(
      String courseId,
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (selectedCourseId == null)
                        ...
                        ..._classes.where((c) => c['id'] == selectedCourseId).map((
                          cls,
                        ) {
                          bool isExpanded = expandedCourses.contains(cls["id"]);
                          List<dynamic> sections = cls["sections"];

                          bool allSectionsSelected = sections.every(
                            (sec) => selectedSections.contains(sec["id"]),
                          );
                          bool someSectionsSelected =
                              sections.any(
                                (sec) => selectedSections.contains(sec["id"]),
                              ) &&
                              !allSectionsSelected;
                                ...
                                Padding(
                                  ...
                                  child: Column(
                                    children: sections.map((sec) {
                                      bool isSecSelected = selectedSections
                                          .contains(sec["id"]);
                      ...
                      Wrap(
                        spacing: 10,
                        children: ['Assignment', 'Project', 'Presentation'].map(
                          (type) {
                          ...
                          Expanded(
                            child: ElevatedButton.icon(
                              ...
                              label: Text(
                                selectedDate == null
                                    ? "Set Deadline"
                                    : "${selectedDate!.month}/${selectedDate!.day} ${selectedDate!.hour}:${selectedDate!.minute.toString().padLeft(2, '0')}",
                                style: TextStyle(
                                  color: Theme.of(
                                    context,
                                  ).colorScheme.secondary,
                              ...
                              ? () async {
                                  try {
                                    final assessmentData = {
                                      ...
                                      'is_group_assignment':
                                          selectedFormat == 'Group',
                                      'file_path': attachedFiles.isNotEmpty
                                          ? attachedFiles[0]['file_path']
                                          : null,
                                    };

                                    await _apiService.createAssessment(
                                      assessmentData,
                                    );
  ...
  Widget build(BuildContext context) {
    var filteredAssessments = _selectedFilter == 'All'
        ...
        : _assessments
              .where(
                (a) =>
                    (a['type'] ??
                        (a['title'].toString().toLowerCase().contains('project')
                            ? 'Project'
                            : 'Assignment')) ==
                    _selectedFilter,
              )
    ...
    if (_selectedCourseIdFilter != null) {
      filteredAssessments = filteredAssessments
          .where((a) => a['course_id'].toString() == _selectedCourseIdFilter)
          .toList();
    }
                  ...
                  SizedBox(
                    ...
                    child: ListView(
                      ...
                      children: [
                        ...
                        Padding(
                          ...
                          child: ChoiceChip(
                            ...
                            labelStyle: TextStyle(
                              color: _selectedCourseIdFilter == null
                                  ? Colors.white
                                  : Colors.black87,
                              fontWeight: _selectedCourseIdFilter == null
                                  ? FontWeight.bold
                                  : FontWeight.normal,
                            ),
                            shape: RoundedRectangleBorder(
                        ...
                        ..._classes.map((cls) {
                          final isSelected =
                              _selectedCourseIdFilter == cls["id"];
                          return Padding(
                            padding: const EdgeInsets.only(right: 10),
                            child: ChoiceChip(
                              avatar: CircleAvatar(
                                backgroundColor: (cls["color"] as Color)
                                    .withOpacity(0.2),
                                child: Text(
                                  cls["initials"],
                                  style: TextStyle(
                              ...
                              label: Text(
                                cls["name"].toString().split(' (')[0],
                              ), // Show title without code for brevity
                              selected: isSelected,
                              onSelected: (selected) {
                                setState(() {
                                  _selectedCourseIdFilter = isSelected
                                      ? null
                                      : cls["id"];
                                });
                              },
  ...
  Widget _buildAssessmentCard(dynamic item) {
    ...
    String type = (title.toLowerCase().contains('project'))
        ? 'Project'
        : (title.toLowerCase().contains('presentation'))
        ? 'Presentation'
        : 'Assignment';

    Color typeColor = type == 'Project'
        ? Colors.purple
        : (type == 'Presentation' ? Colors.orange : Colors.blue);
